...
export function ɵɵtext(index: number, value: string = ''): void {
  ...
  const [isNewlyCreatedNode, textNative] =
      _locateOrCreateTextNode(tView, lView, tNode, adjustedIndex, value);
...
function locateOrCreateTextNodeImpl(
    ...
    [boolean, RText] {
      ...
      if (isCreating) {
        ...
        // hydrating
        textNative = locateNextRNode(ngh, tView, lView, tNode) as RText;
