...
import {DominoAdapter} from './domino_adapter';
...
export const INTERNAL_SERVER_PLATFORM_PROVIDERS: StaticProvider[] = [
  ...
  {provide: PLATFORM_INITIALIZER, useFactory: initDomAdapter, multi: true}, {
...
function initDomAdapter() {
  return () => {
    let adapter = getDOM() as DomAdapter;
    if (!adapter) {
      // TODO: for backwards-compatibility, we may need to
      // fall back to DominoAdapter?
      // DominoAdapter.makeCurrent();
      throw new Error('No DOM adapter provided!');
    }
  };
}
...
function _document(injector: Injector) {
  ...
  if (config && config.document) {
    document = typeof config.document === 'string' ?
        adapter.parseDocument(config.document, config.url) :
        config.document;
  } else {
    document = adapter.createHtmlDocument();
  }
